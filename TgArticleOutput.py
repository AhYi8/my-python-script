...
class TgArticleUtils:
    ...
    link_regex = r'链接：(https://pan\.quark\.cn/s/[0-9a-z]{12})'
    size_regex = r'大小：(.+)'
    tag_regex = r'标签：(.+)'
    invalid_chars = r'\/:*?"<>|'
    ...
    def get_tg_article_content(html_content: str):
        ...
        article = description_meta['content'] if description_meta else ''
        return image, article

    @staticmethod
    def apply_regex(article: str):
        title_match = re.search(TgArticleUtils.title_regex, article)
        description_match = re.search(TgArticleUtils.description_regex, article)
        link_match = re.search(TgArticleUtils.link_regex, article)
        size_match = re.search(TgArticleUtils.size_regex, article)
        tag_match = re.search(TgArticleUtils.tag_regex, article)

        title = TgArticleUtils.sanitize_filename(TgArticleUtils.clean_title(title_match.group(2))) if title_match else ''
    ...
    def append_to_excel(file_name: str, data: tuple):
        if not os.path.exists(file_name):
            ...
            ws.append(['URL', 'Success', 'Title', 'Image', 'Description', 'Link', 'Size', 'Tag'])
            wb.save(file_name)
    ...
    def read_urls_from_txt(file_path: str) -> list:
        try:
            ...
            return []
    @staticmethod
    def process_url(url: str, image_save_path: str, excel_file: str):
        try:
            ...
            image_url, article = TgArticleUtils.get_tg_article_content(html_content)
            if not image_url or not article:
                print(f"无法解析 meta 标签: {url}")
                return None

            # 3. Apply regex to extract information
            title, description, link, size, tag = TgArticleUtils.apply_regex(article)
            if not title or not link:
                print(f"正则解析失败: {url}")
                return None

            # 4. Download the image
            success, renamed_image = TgArticleUtils.download_image(image_url, title, image_save_path)
            if not success:
                print(f"图片下载失败: {url}")
                return None
            ...
            if TgArticleUtils.exists_titles and title in TgArticleUtils.exists_titles:
                print(f"文章已发布：{title}，跳过采集。")
                return None
            # 5. Return all data for appending to Excel
            return (url, success_str, title, renamed_image, description, link, size, tag)
...
def main(concurrency: int = None):
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        future_to_url = {executor.submit(TgArticleUtils.process_url, url, image_save_path, excel_file): url for url in urls}
        total = len(urls)
