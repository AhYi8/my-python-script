...
class ZhanKeArticleUtils:
    ...
    @classmethod
    def php666_article_collect(cls,
                              link_file_path: str = os.path.join(os.getcwd(), 'file', 'un_publish_articles.txt'),
                              xlsx_file_path: str = os.path.join(os.getcwd(), 'file', '666php_article_collect.xlsx'),
                              open_proxy: bool = True,
                              use_local: bool = False) -> List[tuple]:
        links = FileUtils.read_lines(link_file_path, True)
        result: List[tuple] = []
        ...
        for link in links:
            ...
            title = soup.select_one('hgroup.article-info > div.meta > div > h2').get_text(strip=True)
            LogUtils.info(f"正在处理{str(index).zfill(zfill_len)}/{total} --> URL: {link} --> Title: {title}")
            # 获取文章内容的div标签
            content_div = soup.select_one('main.site-main div.entry-wrapper > article')
            for img in content_div.find_all('img'):
                img['class'] = img.get('class', []) + ['aligncenter']
            ...
            # 返回处理后的内容
            processed_content = soup_content.decode_contents()
            FileUtils.append_to_excel(xlsx_file_path, (link, title, processed_content),
                                      ("Link", "Title", "Content"))
            result.append((link, title, processed_content))
        return result
